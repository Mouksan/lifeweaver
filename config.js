...
export const defaultPregnancyData = {
    ...
    _dayRemainder: 0,
    // Здоровье: осложнения разыгрываются один раз при зачатии и раскрываются
    // по мере наступления своей недели (схема вдохновителя).
    _plannedComplications: [],
    complications: [],
    healthStatus: 'normal',
    // Последний тест на беременность
    lastTestResult: null,
    lastTestRpDay: null,
    // Живая динамика от модели (тег RP_STATUS): настроение, либидо, шевеления,
    // размер плода и прочее, что видит модель, но не может знать трекер.
    dynamic: {},
};
...
export const defaultChatData = {
    ...
    timeOfDay: 'day',
    // Динамика детей от модели: { [childId]: { mood, sleep, feeding, note } }
    childDynamic: {},
